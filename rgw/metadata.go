...
func (client *Client) ListMetadataKeys(ctx context.Context, input ListMetadataKeysRequest) (MetadataKeyList, error) {
	if ctx == nil {
		return MetadataKeyList{}, errors.New("rgw: context must not be nil")
	}
	query := url.Values{}
	...
	if input.MaxEntries == nil {
		if err := json.Unmarshal(body, &result.Keys); err != nil {
			return MetadataKeyList{}, fmt.Errorf("rgw: decode %s %s response: %w", request.Method, request.URL.Path, err)
		}
...
func (client *Client) getMetadata(ctx context.Context, section string, query url.Values) (Metadata, error) {
	...
	var result Metadata
	if err := json.Unmarshal(body, &result); err != nil {
		return Metadata{}, fmt.Errorf("rgw: decode %s %s response: %w", request.Method, request.URL.Path, err)
	}
	return result, nil
}
