...
func TestGetUserRateLimit(t *testing.T) {
	...
	configuration, err := client.GetUserRateLimit(ctx, mgr.GetUserRateLimitRequest{UID: fixture.uid})
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpdateUserRateLimit(t *testing.T) {
	...
	if err := client.UpdateUserRateLimit(ctx, mgr.UpdateUserRateLimitRequest{
		UID:       fixture.uid,
		RateLimit: want,
	}); err != nil {
		t.Fatal(err)
