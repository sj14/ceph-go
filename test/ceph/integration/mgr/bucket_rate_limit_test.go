...
func TestGetBucketRateLimit(t *testing.T) {
	...
	configuration, err := client.GetBucketRateLimit(ctx, mgr.GetBucketRateLimitRequest{Name: fixture.name})
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpdateBucketRateLimit(t *testing.T) {
	...
	if err := client.UpdateBucketRateLimit(ctx, mgr.UpdateBucketRateLimitRequest{
		Name:      fixture.name,
		RateLimit: want,
	}); err != nil {
		t.Fatal(err)
