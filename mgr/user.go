...
func (client *Client) GetUser(ctx context.Context, input GetUserRequest) (User, error) {
	if ctx == nil {
		return User{}, errors.New("mgr: context must not be nil")
	}
	if strings.TrimSpace(input.UID) == "" {
		return User{}, errors.New("mgr: user UID must not be empty")
	}
...
func (client *Client) CreateUser(ctx context.Context, input CreateUserRequest) (User, error) {
	...
	if strings.TrimSpace(input.DisplayName) == "" {
		return User{}, errors.New("mgr: user display name must not be empty")
	}
...
func (client *Client) DeleteAccessKey(ctx context.Context, input DeleteAccessKeyRequest) error {
	...
	if strings.TrimSpace(input.AccessKey) == "" {
		return errors.New("mgr: access key must not be empty")
	}
...
func (client *Client) doUserRequest(request *http.Request) (User, error) {
	...
	if err := json.Unmarshal(body, &user); err != nil {
		return User{}, fmt.Errorf("mgr: decode %s %s response: %w", request.Method, request.URL.Path, err)
	}
	return user, nil
}
...
func setUserMutationQuery(query url.Values, email *string, maxBuckets *int64,
	...
	if accountPolicies != nil {
		...
		if err != nil {
			return fmt.Errorf("mgr: encode account policies: %w", err)
		}
