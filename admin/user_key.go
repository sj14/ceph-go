...
func (client *Client) CreateKey(ctx context.Context, input CreateKeyRequest) (CreatedKeys, error) {
	if ctx == nil {
		return CreatedKeys{}, errors.New("admin: context must not be nil")
	}
	if strings.TrimSpace(input.UID) == "" {
		return CreatedKeys{}, errors.New("admin: user UID must not be empty")
	}
	if input.KeyType != UserKeyTypeS3 && input.KeyType != UserKeyTypeSwift {
		return CreatedKeys{}, errors.New("admin: key type must be s3 or swift")
	}
	query := url.Values{
		"key": {""},
		"uid": {input.UID},
	...
	if err != nil {
		return CreatedKeys{}, fmt.Errorf("admin: decode %s %s response: %w", request.Method, request.URL.Path, err)
	}
	return keys, nil
}
