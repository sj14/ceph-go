// Package admin provides a client for Ceph RGW's Admin Ops API.
package admin
...
func WithAdminPath(path string) Option {
	return func(client *Client) error {
		...
		if path == "" {
			return errors.New("admin: admin path must not be empty")
		}
		if strings.Contains(path, "?") || strings.Contains(path, "#") {
			return errors.New("admin: admin path must not include a query or fragment")
		}
...
func WithRegion(region string) Option {
	return func(client *Client) error {
		if strings.TrimSpace(region) == "" {
			return errors.New("admin: region must not be empty")
		}
...
func NewClient(endpoint, accessKey, secretKey string, options ...Option) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("admin: parse endpoint: %w", err)
	}
	if parsedURL.Scheme != "http" && parsedURL.Scheme != "https" {
		return nil, errors.New("admin: endpoint scheme must be http or https")
	}
	if parsedURL.Host == "" {
		return nil, errors.New("admin: endpoint must include a host")
	}
	if parsedURL.RawQuery != "" || parsedURL.Fragment != "" {
		return nil, errors.New("admin: endpoint must not include a query or fragment")
	}
	if strings.TrimSpace(accessKey) == "" {
		return nil, errors.New("admin: access key must not be empty")
	}
	if strings.TrimSpace(secretKey) == "" {
		return nil, errors.New("admin: secret key must not be empty")
	}
	...
	client := &Client{
		...
		userAgent: "rgw-go/admin",
		now:       time.Now,
	}
	for _, option := range options {
		if option == nil {
			return nil, errors.New("admin: option must not be nil")
		}
...
func (client *Client) newRequest(ctx context.Context, method, resource string, query url.Values) (*http.Request, error) {
	...
	if err := client.signer.SignHTTP(
		...
		return nil, fmt.Errorf("admin: sign %s %s request: %w", method, endpoint.Path, err)
	}
	return request, nil
}
...
func (client *Client) do(request *http.Request) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("admin: %s %s: %w", request.Method, request.URL.Path, err)
	}
	defer response.Body.Close()

	body, err := io.ReadAll(io.LimitReader(response.Body, maxResponseBody+1))
	if err != nil {
		return nil, fmt.Errorf("admin: read %s %s response: %w", request.Method, request.URL.Path, err)
	}
	if len(body) > maxResponseBody {
		return nil, fmt.Errorf("admin: %s %s response exceeds %d bytes", request.Method, request.URL.Path, maxResponseBody)
	}
...
func (err *APIError) Error() string {
	...
	return fmt.Sprintf("admin: %s %s returned %d: %s", err.Method, err.Path, err.StatusCode, message)
}
