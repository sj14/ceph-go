...
func userCapabilitiesQuery(uid string, capabilities []Capability) (url.Values, error) {
	if strings.TrimSpace(uid) == "" {
		return nil, errors.New("admin: user UID must not be empty")
	}
	if len(capabilities) == 0 {
		return nil, errors.New("admin: capabilities must not be empty")
	}
	values := make([]string, 0, len(capabilities))
	for _, capability := range capabilities {
		if capability.Type == "" || capability.Permission == "" {
			return nil, errors.New("admin: capability type and permission must not be empty")
		}
